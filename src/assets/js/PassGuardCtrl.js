// (function(a){
export function initocx(){
	var Ajax = "";
	//Windows IE32 
	var PGEdit_IE32_EXE = "YSPassGuardALL.exe";
	var PGEdit_IE32_VERSION = "1.0.0.1";
	//Windows IE64
	var PGEdit_IE64_EXE = "YSPassGuardALL.exe";
	var PGEdit_IE64_VERSION = "1.0.0.1";
	//Windows IE ClassId & ProgId
	var PGEdit_IE_CLASSID = "FBBD7445-A2B7-4A04-9F1D-C71691287FB5";
	var PGEdit_IE_ProgId = "YSPassGuard.PassGuard.1";
	//Windows 非IE插件(NPAPI,用于Chrome42-,FireFox50-)
	var PGEdit_FF = "YSPassGuardALL.exe";
	var PGEdit_FF_VERSION = "1.0.0.1"; 
	//Windows 非IE插件 Mime类型和PluginName
	var PGEdit_FF_MIMETYPE = "pass-guard-ys";
	var PGEdit_FF_PluginName = "YSPassGuard";
	//Mac OS 插件版(NPAPI,用于Safari,FireFox50-,Chrome42-)
	var PGEdit_MacOs = "YSZFMac.pkg";
	var PGEdit_MacOs_VERSION = "1.0.0.1";
	//Mac OS 插件版 Mime类型和PluginName
	var PGEdit_MacOs_MIMETYPE = "pass-guard-ys";
	var PGEdit_MacOs_PluginName = "YSPassGuard";
	//Windows & Mac OS 非插件(用于Chrome42+,FireFox50+,MircroSoft Edge)
	var PGEdit_Edge = "YSPassGuardALL.exe";
	var PGEdit_Edge_Mac = "YSZFEdge.pkg";
	var PGEdit_Edge_VERSION = "1.0.0.1";
	//生产license授权
	var license = "";
	var licenseMac = "";
	//测试license授权
	...
	//生产
	var urlPar = "aHR0cDovL2EueXNlcGF5LmNvbQ==";
	//测试
	...
	var PGEdit_Edge_URL = "https://windows10.microdone.cn",port = 5961,lastPort = port + 4;
	...
	pge.prototype.pwdResultSM4 =function(){
		var code ='';
		if(!isInstalled){
			code = '';
		}else{
			try{
				...
				if(this.osBrowser ==1||this.osBrowser ==3){
					...
					OUTJSON.id = this.settings.pgeWindowID,OUTJSON.data.datatype = 7,OUTJSON.data.encrypttype = 8;
					code = this.pwdGetData(OUTJSON,"2008");
				}
			}catch(err){code ='';}
		}
	...
	pge.prototype.getPgeHtml = function(){
		if(this.osBrowser ==1||this.osBrowser ==3){
			...
			/*if((this.getConvertVersion(objVersion)<this.getConvertVersion(PGEdit_Edge_VERSION))&&PGEdit_Update=="1"){
				...
				return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-align:center;line-height:45px;"><a target="_blank" href="'+this.settings.pgePath+this.pgeditFFExe+'">'+this.pgeDownText+'</a></div>';
			}*/
		//else{
				return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-align:center;line-height:45px;"><a target="_blank" href="'+this.settings.pgePath+this.pgeditFFExe+'">'+this.pgeDownText+'</a></div>';
			//}
	...
	pge.prototype.getDownHtml =function(){
		if(this.osBrowser ==1 || this.osBrowser ==3){
			return '<div id="'+this.settings.pgeId+'_down" class="'+this.settings.pgeInstallClass+'" style="text-align:center;line-height:45px;"><a target="_blank" href="'+this.settings.pgePath+this.pgeditIEExe+'">'+this.pgeDownText+'</a></div>'  ;
	...
	//检测控件是否已安装(外部接口)
	pge.prototype.checkInstallNew = function(){
		try{
			if(this.osBrowser ==1 || this.osBrowser ==3){
				var comActiveX = new ActiveXObject(PGEdit_IE_ProgId);
				return true;
				...
				var id = CIJSON.id = this.settings.pgeWindowID;
				urls = PGEdit_Edge_URL + ":" + port;
				 var datac = this.getEnStr(this.settings.pgeRZRandNum,CIJSON);
				 var RZCIJSON = {"rankey":this.settings.pgeRZRandNum,"datab":this.settings.pgeRZDataB,"datac":datac};
				var  datas = JSON.stringify(RZCIJSON).replace(/\+/g,"%2B");
				Ajax.request({
					 url : urls,
					 timeout :2000,
					 type : "GET",
	...
	pge.prototype.openProt = function(id,inputID){
		...
		if(typeof onceInterv[id] =="string" ){//监听焦点切出
			...
			onceInterv[id] =window.setInterval("pgeCtrl.intervlOut(\""+id+"\",\""+inputID+"\")",1000);
...
/* 初始化密码控件时封装方法 */
export function ocxInit(id,ocxRandNum){
    //new 控件对象
	var pgeditor = new pge({
		pgePath : "https://newportal.ysepay.com/agent/ocx/",//控件下载目录，可以指定绝对路径，如"http://www.baidu.com/download/"
		pgeId : id,//控件id
		pgeEditType : 0,//控件显示类型,0(星号),1(明文)
		pgeEreg1 : "[\\s\\S]*",//输入过程中字符类型限制，如"[0-9]*"表示只能输入数字
		pgeEreg2 : "^(?=[a-zA-Z0-9]*(?:[a-zA-Z][0-9]|[0-9][a-zA-Z]))[a-zA-Z0-9]{8,20}$",//输入完毕后字符类型判断条件，与pgeditor.pwdValid()方法对应
		...
		AffineX:"ff0bd326bbfaa96fa381ab085be2c052a6075f9ac10b40c3d87241219bb553b0",
		AffineY:"1a36e52fee17a9ec928a868992bbeebd9e9f8b2a8609fed77eb33fb763d96b29",
		pgeWindowID : new Date().getTime() + 1+ id,
		pgeRZRandNum : ocxRandNum.pgeRZRandNum,
		pgeRZDataB : ocxRandNum.pgeRZDataB
	});
	// 定义公共对象
