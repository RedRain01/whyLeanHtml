...
export default {
    ...
    mounted(){
			initocx();
			getInitRandom().then((result) => {
				this.pgeditor = ocxInit('_password',result.data);
				this.box = this.pgeditor.load();
				//初始化
				pgeInit();
			})
		},
      ...
      submit(){
					...
					getMcryptKey().then(response => {
						...
						login(data).then(result=>{
							if (result.status=="success") {
								...
								this.$router.push({name:"orderingInfo"});
							}else{
			...
			// 兼容ie interval
			/* timeoutForInterval(n){
				setTimeout(() => {
					if (window.pges.length==n) {
						setTimeout(() => {
							//初始化
							pgeInit();
						}, 100);
					}else{
						this.timeoutForInterval(n);
					}
				}, 100);
			}, */
		},
...
.container { margin:0 auto 0 auto; padding-top: 10%; position: relative; width: 900px; }
#content {
	background: #f9f9f9;
...
#content #form #password{
	margin: 0 8px 10px;
	padding: 1px 10px 0px 45px;
	width: auto;
}
#content #form input[type="text"]:hover,
#content #form #password:hover {
	-webkit-box-shadow: 0 0 2px #FDC156 inset;
	-moz-box-shadow: 0 0 2px #FDC156 inset;
	-ms-box-shadow: 0 0 2px #FDC156 inset;
	-o-box-shadow: 0 0 2px #FDC156 inset;
	box-shadow: 0 0 2px #FDC156 inset;
	background-color: #fff;
	border: 1px solid #FDC156;
}
